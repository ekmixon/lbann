...
class WAE(lbann.modules.Module):
    ...
    def __init__(self, encoder_out_dim, decoder_out_dim, name=None):
        self.instance = 0
        self.name = name or 'wae{0}'.format(WAE.global_count)
        ...
        decoder_neurons = [64,128,256]
           ...
           #Encoder
        self.enc_fc0 = fc(
            encoder_neurons[0], activation=lbann.Elu, name=f'{self.name}_enc_fc0'
        )

        self.enc_fc1 = fc(
            encoder_neurons[1], activation=lbann.Tanh, name=f'{self.name}_enc_fc1'
        )

        self.enc_fc2 = fc(
            encoder_neurons[2], activation=lbann.Tanh, name=f'{self.name}_enc_fc2'
        )
        ...
        self.enc_out = fc(encoder_out_dim,name='enc_out')
           ...
           #Decoder
        self.dec_fc0 = fc(
            decoder_neurons[0], activation=lbann.Elu, name=f'{self.name}_dec_fc0'
        )

        self.dec_fc1 = fc(
            decoder_neurons[1], activation=lbann.Tanh, name=f'{self.name}_dec_fc1'
        )

        self.dec_fc2 = fc(
            decoder_neurons[2], activation=lbann.Tanh, name=f'{self.name}_dec_fc2'
        )
