...
class TrackModule(lbann.modules.Module):
    ...
    def __init__(self,neuron_dims=[1000,1000,1000],activation=lbann.Relu, keep_prob=0.95, weights=[], name=None):
        self.instance = 0
        self.name = name or 'combo{0}'.format(TrackModule.global_count)
        ...
        if weights:
            self.track_fc = [
                fc(
                    ...
                    weights=[weights[2 * i], weights[2 * i + 1]],
                    name=f'{self.name}fc{str(i)}',
                )
...
class Combo(lbann.modules.Module):
    ...
    def __init__(self,neuron_dims=[1000,1000,1000], activation=lbann.Relu, keep_prob=0.95,  name=None):
        self.instance = 0
        self.name = name or 'combo{0}'.format(Combo.global_count)
            ...
            shared_w.extend(
                (
                    lbann.Weights(
                        ...
                        name=f'drug_matrix{str(i)}',
                    ),
                    lbann.Weights(
                        initializer=lbann.ConstantInitializer(value=0.0),
                        name=f'drug_bias{str(i)}',
                    ),
                )
            )
        ...
        self.geneT = TrackModule(
            neuron_dims, activation, keep_prob, name=f'{self.name}gene_track'
        )

        self.drug1T = TrackModule(
            neuron_dims,
            activation,
            keep_prob,
            shared_w,
            name=f'{self.name}drug1_track',
        )

        self.drug2T = TrackModule(
            neuron_dims,
            activation,
            keep_prob,
            shared_w,
            name=f'{self.name}drug2_track',
        )

        self.concatT = TrackModule(
            neuron_dims, activation, keep_prob, name=f'{self.name}concat_track'
        )
    ...
    def forward(self,x):
        ...
        concat = self.concatT(
            lbann.Concatenation([gene, drug1, drug2], name=f'{self.name}concat')
        )

        return lbann.FullyConnected(concat,num_neurons = 1, has_bias = True)
